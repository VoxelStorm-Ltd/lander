...
void universe::make_explosion(Vector3d coords, double energy) {
  ...
  if(energy > 1000000000000000000000000000000.0) {    // supernova
    std::cout << "INFO: The incredible impact produced "
    ...
    std::cout << "INFO: The entire " << currentsystem->get_name() << " star system is destroyed!" << std::endl;
    ...
    std::cout << "INFO: The impact produces an blinding explosion, releasing " << energy / 420000000000000000000000.0 << " times as much energy as the asteroid impact that wiped out the dinosaurs." << std::endl;
    std::cout << "INFO: The flash lights up the entirety of the " << currentsystem->get_name() << ", and the afterglow persists for days." << std::endl;
    ...
    std::cout << "INFO: The impact produces an incredible fireball, releasing " << energy / 2510000000000000000000.0 << " times as much energy as the Shoemaker-Levy 9 comet's impact on Jupiter." << std::endl;
    std::cout << "INFO: The flash is seen even in daylight on every planet in the " << currentsystem->get_name() << "." << std::endl;
    destroy_range(coords, 300000.0);
  } else if(energy > 209200000000000000.0) {          // 50 megaton TNT
    std::cout << "INFO: The impact produces an incredible fireball, releasing " << energy / 209200000000000000.0 << " times as much energy as the largest nuclear bomb ever detonated." << std::endl;
    std::cout << "INFO: The damage extends for hundreds of kilometers." << std::endl;
    destroy_range(coords, 300000.0);
  } else if(energy > 125520000000000000.0) {             // Tunguska event
    std::cout << "INFO: The impact produced a massive fireball, releasing " << energy / 125520000000000000.0 << " times as much energy as the Tunguska impact event." << std::endl;
    std::cout << "INFO: Extensive damage occurs in the surrounding area." << std::endl;
    destroy_range(coords, 200000.0);
  } else if(energy > 4184000000000000.0) {               // 1 megaton TNT
    std::cout << "INFO: The impact produced a large fireball, the equivalent of " << energy / 4184000000000000.0 << " megatons of TNT." << std::endl;
  ...
  for(auto const &it : currentsystem->bodies) {
    ...
    if(thisastro) {
      if(Vector3d(thisastro->position - coords).length() < range) {
        std::cout << "INFO: " << thisastro->get_name() << " perished in the fireball." << std::endl;
        thisastro->kill();
        continue;
      }
    }
    ...
    if(thiscraft) {
      ...
      if(distance < range / 2.0) {
        std::cout << "INFO: " << thiscraft->get_name() << "'s shielding could not withstand the explosion." << std::endl;
        thiscraft->destroy();
